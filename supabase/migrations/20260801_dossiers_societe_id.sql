-- Dossiers : rattachement obligatoire à une société (SLTT / Top Doumani).
-- Rétrocompat : backfill vers la société transit (is_transit), puis NOT NULL.
-- Classeur : les mouvements dossier utilisent d.societe_id au lieu du hardcode transit.
...
create index if not exists idx_dossiers_societe_id on public.dossiers(societe_id);
