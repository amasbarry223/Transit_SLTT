-- Le bloc "ecritures" de classeur_mouvements exclut déjà les écritures liées
-- à un dossier (where e.dossier_id is null), pour éviter de compter deux fois
-- la même créance : une fois comme ligne "Dossier" (montant_investi/montant_paye
-- du dossier) et une fois comme ligne "Paiement". Le même filtre a été omis
-- par erreur sur le bloc "factures" : une facture liée à un dossier facturé
-- apparaissait à la fois comme ligne "Dossier" et comme ligne "Facture",
-- doublant le solde dû affiché au client (Classeur, KPI fiche client, export,
-- message de relance auto-généré).
