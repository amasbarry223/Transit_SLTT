...
```bash
# Option 1 : Via le workspace monorepo à la racine
npm run dev:back

# Option 2 : En se plaçant dans le dossier backend/
cd backend
npm install
npm run start:dev
```
...
# Option 2 : En se plaçant dans le dossier frontend/
cd frontend
npm install
npm run dev
```
...
### 5. Commandes Monorepo Utiles (depuis la racine)

```bash
# Lancer les tests de tout le projet (front + back)
npm run test

# Compiler les deux projets
npm run build

# Lancer les vérifications de linting
npm run lint
```
