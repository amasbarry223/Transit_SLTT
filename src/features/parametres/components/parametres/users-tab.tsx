...
export function UsersTab() {
  ...
  // Nombre d'administrateurs actifs — sert à empêcher que la dernière personne
  // habilitée à gérer les comptes soit désactivée, supprimée ou rétrogradée,
  // ce qui verrouillerait l'application (plus personne ne pourrait redonner
  // les droits d'administration).
  const activeAdminCount = useMemo(
    () => users.filter((u) => u.role === "Administrateur" && u.actif).length,
    [users],
  );
  ...
  function actionDisabledReason(u: UserAccount): string | undefined {
    ...
    if (u.role === "Administrateur" && u.actif && activeAdminCount <= 1) {
      return "Impossible de modifier le dernier administrateur actif.";
    }
    return undefined;
  }
  ...
  async function handleEdit(id: string, state: UserFormState) {
    ...
    if (
      ...
      toastWarning(toast, {
        title: "Impossible de retirer le dernier administrateur",
        description: "Promouvez un autre utilisateur au rôle Administrateur avant de modifier celui-ci.",
      });
      return;
    }
    const input: UserInput = {
  ...
  async function handleToggleActive(u: UserAccount) {
    ...
    if (u.actif) {
      if (u.role === "Administrateur" && activeAdminCount <= 1) {
        toastWarning(toast, {
          title: "Impossible de désactiver le dernier administrateur",
          description: "Un compte Administrateur actif doit toujours rester disponible pour gérer les accès.",
        });
        return;
      }
  ...
  return (
    <div className="space-y-5">
      ...
      <ConfirmDeleteDialog
        ...
        onConfirm={async () => {
          if (!deleteId) return;
          if (userToDelete?.role === "Administrateur" && userToDelete.actif && activeAdminCount <= 1) {
            toastWarning(toast, {
              title: "Impossible de supprimer le dernier administrateur",
              description: "Promouvez un autre utilisateur au rôle Administrateur avant de le supprimer.",
            });
            setDeleteId(null);
            return;
          }
