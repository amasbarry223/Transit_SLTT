    ...
    <TabsContent value="marchandise" className="mt-0 space-y-6">
      ...
      <Card className="gap-0 overflow-hidden p-0 shadow-sm border-border/80">
        ...
        {filters.filtered.length === 0 ? (
          <EmptyState
            icon={ClipboardList}
            title={filters.hasActiveFilters ? UI.empty.bons.filtered.title : UI.empty.bons.zero.title}
            description={filters.hasActiveFilters ? UI.empty.bons.filtered.description : UI.empty.bons.zero.description}
            action={
              !filters.hasActiveFilters && canWrite ? (
                <Button onClick={onOpenCreateDialog}>
                  <Plus className="size-4" />
                  {UI.empty.bons.zero.action}
                </Button>
              ) : undefined
            }
          />
