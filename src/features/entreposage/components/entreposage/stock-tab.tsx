    ...
    <div className="space-y-4">
      ...
      <Card className="gap-0 overflow-hidden p-0 shadow-sm border-border/80">
        {filtered.length === 0 ? (
          <EmptyState
            icon={Package}
            title={stock.length === 0 ? UI.empty.stock.zero.title : UI.empty.stock.filtered.title}
            description={stock.length === 0 ? UI.empty.stock.zero.description : UI.empty.stock.filtered.description}
            primaryAction={
              stock.length === 0 && canWrite && onCreateItem
                ? { label: UI.empty.stock.zero.action, onClick: onCreateItem, icon: Plus }
                : undefined
            }
          />
        ) : (
