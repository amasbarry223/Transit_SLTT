...
export function useEcrituresScreen() {
  ...
  const [submittingNewEcriture, setSubmittingNewEcriture] = useState(false);
  ...
  async function validatePayment() {
    if (!selected) return;
    // Garde-fou contre le double clic : "recordPayment" est un appel réseau
    // asynchrone et le bouton restait cliquable pendant l'attente, ce qui
    // pouvait enregistrer deux fois le même paiement.
  ...
  async function createEcriture() {
    // Garde-fou contre le double clic pendant l'appel réseau ci-dessous.
    if (submittingNewEcriture) return;
    if (!neClientId) {
    ...
    try {
      ...
      toastError(toast, error, {
        ...
        fallback: "L'écriture n'a pas pu être créée.",
      });
      return;
    } finally {
      setSubmittingNewEcriture(false);
    }
  ...
  return {
    ...
    datePaiement, setDatePaiement, note, setNote, openPayment, validatePayment, submittingPayment,
    ...
    openNewEcriture, createEcriture, submittingNewEcriture, exportExcel,
  };
}
