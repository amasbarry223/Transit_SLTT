...
export function ComptabiliteGeneraleImportDialog({ open, onOpenChange, entite, initialFile }: ImportDialogProps) {
  ...
  // réconcilié — cf. le même garde-fou dans OperationFormDialog (saisie manuelle).
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      setRows(
        parsed.map((r, i) => {
          const clotured = !!(r.date && derniereClotureFin && r.date <= derniereClotureFin);
          const blocking = !r.date || !r.type || !r.clientNom || !r.nature || clotured;
          ...
          <div className="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto p-6">
                      ...
                      <TableRow key={r.key} className={cn(r.blocking && "bg-red-50/60 dark:bg-red-950/20", !r.blocking && !r.selected && "opacity-45")}>
                        ...
                        <TableCell className="text-xs">
                          {r.date && derniereClotureFin && r.date <= derniereClotureFin ? (
                            <span className="font-medium text-red-600 dark:text-red-400" title={`Période clôturée jusqu'au ${derniereClotureFin}`}>
                              {r.date} (clôturée)
                            </span>
                          ) : (
            ...
            {blockingCount > 0 && (
              <div className="flex items-start gap-2 rounded-lg border border-red-300 bg-red-50 px-3 py-2 text-sm text-red-900 dark:border-red-800 dark:bg-red-950/40 dark:text-red-200">
                ...
                <p>
                  {blockingCount} ligne{blockingCount !== 1 ? "s" : ""} ne peu{blockingCount !== 1 ? "vent" : "t"} pas être importée{blockingCount !== 1 ? "s" : ""} en l&apos;état — corrigez le fichier source (date, tiers, nature ou montant manquant){" "}
                  {derniereClotureFin
                    ? `ou choisissez une date postérieure à la dernière clôture (${derniereClotureFin}) `
                    : ""}
                  et réimportez.
                </p>
              </div>
