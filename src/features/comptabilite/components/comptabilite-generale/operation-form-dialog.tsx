...
export function OperationFormDialog({ open, onOpenChange, entite }: OperationFormDialogProps) {
  ...
  // clôturée (le solde théorique déjà réconcilié deviendrait faux après coup
  // sans que la clôture existante ne soit corrigée).
  ...
  async function handleSubmit() {
    ...
    if (derniereClotureFin && date <= derniereClotureFin) {
      toastWarning(toast, {
        title: "Période déjà clôturée",
        description: `Cette date (${formatDateShort(date)}) est antérieure ou égale à la dernière clôture (${formatDateShort(derniereClotureFin)}). Choisissez une date postérieure ou annulez la clôture avant de corriger cette période.`,
      });
      return;
    }
    setSubmitting(true);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              ...
              {derniereClotureFin && date <= derniereClotureFin && (
                <p className="text-xs text-red-600 dark:text-red-400">
                  Période clôturée jusqu&apos;au {formatDateShort(derniereClotureFin)}.
                </p>
              )}
            </div>
            {!isTopDoumani && (
        ...
        <DialogFooter className="gap-2 sm:gap-0">
          ...
          <Button onClick={handleSubmit} disabled={submitting || !!(derniereClotureFin && date <= derniereClotureFin)}>Enregistrer</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
