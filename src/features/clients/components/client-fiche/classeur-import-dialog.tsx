...
export function ClasseurImportDialog({
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      toastError(toast, e, { title: "Import impossible", fallback: "Fichier Excel invalide" });
    } finally {
      setParsing(false);
    }
  }
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      onApplied();
      toastSuccess(toast, { title: "Import Excel terminé", description: `${applied} ligne(s) appliquée(s)${plan.unmatched.length ? ` · ${plan.unmatched.length} non appariée(s)` : ""}.` });
      onOpenChange(false);
      reset();
    } catch (e) {
