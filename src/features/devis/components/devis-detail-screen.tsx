...
export function DevisDetailScreen() {
  ...
  async function applyStatut(statut: DevisStatut) {
    ...
    try {
      ...
      toastSuccess(toast, { title: "Statut mis à jour", description: `${d.reference} → ${statut}` });
    } catch (error) {
      toastWarning(toast, {
        title: "Transition impossible",
        description: error instanceof Error ? error.message : "Cette transition de statut n'est pas autorisée.",
      });
    }
  };
  const handlePrint = () => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toastSuccess(toast, { title: "Devis supprimé", description: devis.reference });
      go("devis");
    } catch (error) {
      toastError(toast, error, {
        title: "Impossible de supprimer le devis",
