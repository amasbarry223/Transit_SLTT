  ...
  return (
    ...
    >
      ...
      <TableCell className="px-4 py-3.5 text-right tabular-nums">
        ...
        <span className="ml-1 text-xs text-muted-foreground">{item.unite}</span>
      </TableCell>
      <TableCell className="hidden px-4 py-3.5 text-muted-foreground md:table-cell">
        {item.depositaire}
      </TableCell>
        ...
        <div className="flex items-center justify-end gap-1">
          ...
          {canWrite && onDelete && (
            <Button
              variant="ghost"
              size="icon"
              className="size-11 text-slate-400 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-950/40"
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          ...
          {item.clientId && item.clientNom && onOpenClient ? (
            ...
            <p className="text-xs text-muted-foreground">{item.depositaire}</p>
          )}
        </div>
        <StockStatutBadge statut={statut} />
      </div>
      <dl className="mt-3 space-y-1.5 text-sm">
        <div className="flex justify-between gap-3">
          ...
          <dd className="tabular-nums font-semibold text-foreground">
            {item.quantite} <span className="text-xs font-normal text-muted-foreground">{item.unite}</span>
          </dd>
        </div>
        <div className="flex justify-between gap-3">
        ...
        <div className="flex justify-between gap-3">
          ...
          <dd className="tabular-nums text-foreground/90">{formatFCFA(item.sommePayee)}</dd>
        </div>
        <div className="flex justify-between gap-3">
      ...
      <div className="mt-3 flex flex-wrap justify-end gap-2 border-t border-border pt-3">
        ...
        {canWrite && onDelete && (
          <Button
            variant="ghost"
            size="icon"
            className="size-9 text-slate-400 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-950/40"
            aria-label="Supprimer l'article"
            title="Supprimer"
            onClick={() => onDelete(item.id)}
          >
