...
function documentFileFilter(
  ...
  if (!ALLOWED_DOCUMENT_EXTENSIONS.has(ext)) {
    cb(new BadRequestException(`Extension de fichier non autorisée (${ext || 'sans extension'}).`), false);
    return;
  }
  const mime = file.mimetype?.toLowerCase();
  if (!mime || !ALLOWED_DOCUMENT_MIMETYPES.has(mime)) {
    cb(new BadRequestException(`Type MIME non autorisé (${mime || 'inconnu'}).`), false);
    return;
  }
  cb(null, true);
}
